...
bool testHMAC()
{
	...
	// Ensure the size of referenceMac matches HMAC_SIZE_BYTES
	static_assert(sizeof(referenceMac) == HMAC_SIZE_BYTES, "Size of referenceMac does not match HMAC_SIZE_BYTES");
...
bool readRsaKeypair(const char* keypairFilename, const char* password, const char* certFilename, EVP_PKEY_CTX** pPrivateKeyContext, EVP_PKEY_CTX** pPublicKeyContext)
{
...
bool testRsaSigning(EVP_PKEY_CTX* privateKeyContext, EVP_PKEY_CTX* publicKeyContext, bool toModifyMessage)
{
...
bool testRsa(bool toModifyMessage)
{
	...
	EVP_PKEY_CTX* publicKeyContext = NULL;
...
bool testCertificateChecking(const char* certFilename, const char* caCertFilename, const char* expectedCN)
{
    ...
    {
        printf("Error reading CA certificate filename - %s\n", caCertFilename);
        return false;
    }
    ...
    if (!result)
    {
        printf("Certificate check failed for cert: %s, CA: %s, expected CN: %s\n", certFilename, caCertFilename, expectedCN);
    }
    return result;
}
...
int main(int argc, char** argv)
{
	...
	if (testCertificateChecking("bob.crt", "rootCA.crt", "Bob.com"))
